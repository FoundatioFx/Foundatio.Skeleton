...
namespace Foundatio.Skeleton.Core.Collections {
    public class DataDictionary : Dictionary<string, object> {
        ...
        public T GetValueOrDefault<T>(string key, T defaultValue = default(T)) {
            ...
            if (data is string) {
                try {
                    return JsonConvert.DeserializeObject<T>((string)data, JsonHelper.DefaultSerializerSettings);
                } catch { }
            }

            if (data is JObject || data is JArray) {
                try {
                    return JsonConvert.DeserializeObject<T>(data.ToString(), JsonHelper.DefaultSerializerSettings);
                } catch { }
            }
