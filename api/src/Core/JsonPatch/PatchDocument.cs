...
namespace Foundatio.Skeleton.Core.JsonPatch {
    ...
    public class PatchDocument {
        ...
        public static PatchDocument Load(JArray document) {
            ...
            foreach (var jOperation in document.Children()) {
                var jObject = jOperation as JObject;
                if (jObject != null) {
                    var op = Operation.Build(jObject);
        ...
        public static PatchDocument Parse(string jsondocument) {
            var token = JToken.Parse(jsondocument);
            var jobject = token as JObject;

            var root = jobject != null
                ? new JArray(jobject)
                : token as JArray;
